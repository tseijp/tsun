...
import {entryAtom, rowAtom, indexAtom} from '../atoms'
...
export function useCamera (): [
    ...
    useDrag(({ active, cancel, movement: [mx, my], vxvy: [vx, vy]}) => {
        ...
        const x = swipeY? index: swipeX
            ? clamp(index + swipeX, 0, entry.length - 1)
            : index + (active? -mx / iw: 0)
        const y = swipeX? row: swipeY
            ? clamp(row + swipeY, 0, pages.length - 1)
            : row + (active? my / ih: 0)
        return (!swipeX && !swipeY)
            ? void (setSpring({x, y}))
            : void (setIndex(x), setRow(y), cancel())
    }, {domTarget: window})
